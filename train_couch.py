...
f=__file__
f=f.replace('train_couch.py', '')
root=f+'Couch'
root_dir = os.path.join(root)

img_dir = os.path.join(root_dir+'/All Couch')
...
model.save(root_dir+"/my1.h5")
img_vector_features=np.array(img_vector_features)
img_paths=np.array(img_paths)
np.save(root_dir+'/n2.npy',img_vector_features)
np.save(root_dir+'/n3.npy',img_paths)
